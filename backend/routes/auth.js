...
router.put("/change-password", authenticateToken, async (req, res) => {
  try {
    ...
    if (user.googleId || user.authProvider === "google") {
      return res.status(400).json({
        message:
          "Password cannot be changed for Google accounts. Please change your password through your Google Account settings.",
      });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role === "admin" && user.role === "super_admin") {
        return res
          .status(403)
          .json({ message: "Cannot modify super admin status" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (user.role === "super_admin" && req.user._id.toString() !== userId) {
        return res
          .status(403)
          .json({ message: "Cannot delete other super admin accounts" });
      }
